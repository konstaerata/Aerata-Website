...
| **aerata.gr domain split** (2026-09-01) | Greek's canonical home moved from `aerata.com/el/...` to `aerata.gr` at the URL root. New shared `siteOrigins.js` module maps el→aerata.gr (root), en/nl stay under aerata.com. Legacy `/el/` routes still render but canonical-demote to the `.gr` equivalent. Athens `LocalBusiness` schema now points at `aerata.gr`. New `public/robots-gr.txt` served on the `.gr` host via a Netlify host-conditional redirect (robots.txt can't be made host-aware client-side — it's fetched directly by crawlers). New `scripts/check-domain-seo.js` (wired as `npm run check:domain-seo`, run in CI after build) asserts the canonical/hreflang/sitemap output never leaks a legacy `/el/` URL. Found and fixed a real bug during verification: `SiteLayout.jsx` was passing the raw `:lang` URL param (not `LanguageProvider`'s hostname-aware resolved language) into `<html lang>` — meant `aerata.gr` pages would have rendered Greek content under `<html lang="en">`. Verified end-to-end via Playwright with real cross-origin request routing (not `window.location` property overrides, which real browsers reject) across `.gr`, legacy `.com/el/`, `.com`, and `.com/nl/`. See `seo-strategy.md`'s Multilingual strategy section for full details and the internal-links scope decision (unchanged from the original `/nl/`/`/el/` work — only the language switcher is domain-aware). | `src/lib/siteOrigins.js` (new), `src/components/SEO.jsx`, `src/lib/schemas.js`, `src/lib/LanguageContext.jsx`, `src/components/layout/SiteLayout.jsx`, `scripts/generate-sitemap.js`, `netlify.toml`, `public/robots-gr.txt` (new), `scripts/check-domain-seo.js` (new) |
...
4. **Locale-preserving internal links** — currently only the language switcher navigates between locale prefixes/domains; other internal links always point at the English/`aerata.com` URL, including for `aerata.gr` visitors. Revisit if user behavior data (GA4, now live) shows this causes meaningful drop-off for non-English visitors.
5. **Hard redirect legacy `/el/` to `aerata.gr`** — currently canonical-demoted but still live (see the domain-split entry above). Revisit once `aerata.gr` has its own indexing/traffic history and there's confidence nothing still depends on the `.com/el/...` URLs.
