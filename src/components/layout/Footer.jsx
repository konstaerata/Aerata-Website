...
function isOfficeOpen(timezone) {
  const now = new Date();
  const hour = parseInt(
...
function LiveClock({ timezone, label, openLabel = 'Open', closedLabel = 'Closed' }) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs text-white/60">
      <span className={`w-2 h-2 rounded-full ${open ? 'bg-lime animate-pulse' : 'bg-red-400'}`} />
      <span>{label}</span>
      <span className="font-mono text-lime">{time}</span>
      <span className={`text-[10px] font-mono uppercase tracking-wide ${open ? 'text-lime/70' : 'text-red-400/70'}`}>
    ...
    <footer className="bg-primary text-primary-foreground border-t border-primary/20">
      ...
      <div className="border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <h2 className="font-oxanium text-3xl md:text-4xl font-bold text-white mb-4 tracking-tight">
            {t('footer.ctaTitle')}
          </h2>
          ...
          <Link to="/contact"
            className="inline-flex items-center px-8 py-3 font-oxanium font-semibold text-sm tracking-wide bg-lime text-primary hover:bg-lime/90 transition-all duration-300 rounded glow-pulse cursor-pointer"
          >
            {t('footer.ctaButton')}
          </Link>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          {/* HQ */}
          <div>
            <h4 className="font-oxanium font-semibold text-white text-sm tracking-wider uppercase mb-4">{t('footer.hqTitle')}</h4>
            ...
            <div className="mt-4">
              <LiveClock timezone="Europe/Amsterdam" label="Delft" openLabel={t('footer.open')} closedLabel={t('footer.closed')} />
            </div>
          </div>

          {/* Athens */}
          <div>
            <h4 className="font-oxanium font-semibold text-white text-sm tracking-wider uppercase mb-4">{t('footer.athensTitle')}</h4>
            <div className="space-y-3 text-sm text-white/70">
            ...
            <div className="mt-4">
              <LiveClock timezone="Europe/Athens" label="Athens" openLabel={t('footer.open')} closedLabel={t('footer.closed')} />
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="font-oxanium font-semibold text-white text-sm tracking-wider uppercase mb-4">{t('footer.quickLinks')}</h4>
        ...
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col gap-3">
          ...
          <div className="flex flex-wrap justify-center sm:justify-start gap-x-4 gap-y-1 text-[11px] text-white/35 font-mono">
            <span>VAT: NL863319932B01</span>
            <span className="hidden sm:inline text-white/20">|</span>
            <span>KVK: 84696060</span>
            <span className="hidden sm:inline text-white/20">|</span>
            <span>NL RDW Operator Reg: NLD-OP-[PENDING]</span>
          </div>
        </div>
      </div>
    </footer>
